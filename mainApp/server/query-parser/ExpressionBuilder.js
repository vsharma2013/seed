...
function ExpressionBuilder(headerJson){
	if(headerJson.dateRange){
		this.sysStartDate = headerJson.dateRange.startDate;
		this.sysEndDate = headerJson.dateRange.endDate;
	}
	else{
		this.sysStartDate = '2003/01/01';
		this.sysEndDate = '2016/3/31';
	}
	this.sysMaxYear = new Date(this.sysEndDate).getFullYear();
}

ExpressionBuilder.prototype.getDateRange = function(){
	return {'startDate':this.sysStartDate,'endDate':this.sysEndDate};
}
